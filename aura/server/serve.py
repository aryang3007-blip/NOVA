...
# ── /phone — terminal alias for the canonical DEVICE COMMANDS ─────────────
# The chat "/devices ..." subcommands, the bridge device_command action and
# this terminal "/phone" command all run ONE function: devices.command().
# That keeps the companion control surface identical everywhere and the AI
# path untouched.
def _phone_cli(arg=""):
    """Printable lines for the terminal. Delegates to devices.command()."""
    if devices is None:
        return ["  Device gateway unavailable (server/devices.py failed to import)."]
    parts = (arg or "").split(maxsplit=1)
    sub = parts[0].lower() if parts else "help"
    rest = parts[1].strip() if len(parts) > 1 else ""
    if sub == "devices":          # terminal alias: /phone devices == /devices list
        sub = "list"
    r = devices.command(sub, rest)
    lines = [""]
    if sub in ("", "help"):
        lines.append(c(36, "── DEVICE COMMANDS (also available as /devices in chat) ─────────"))
        lines.append("")
    lines += str(r.get("message", "")).splitlines()
    if not r.get("ok"):
        lines.insert(0, c(31, "✗"))
    return lines
